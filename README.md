...
- 7 个页面：第一人称体验、货架橱窗、收藏、商品详情、设置、钱包流水、历史情境。
- Canvas 原创绘制的公司厕所和阳台场景、双手、打火机、烟支、烟雾和抖灰反馈。
- 点燃、长按吸入、松开、自然燃烧、抖灰、结束；音效与震动可关闭。
- 当前时间自动获取，用户选地点；事件按北京时间、地点、星期、权重和冷却生成。
- 三款原创外观与四款品牌参考包装：中华（硬）、钻石（荷花）、玉溪（软）、白沙（硬）。支持虚拟余额兑换、库存、基础款、每日补给、偏好同步、进度保存与恢复。包装图与授权状态见 [商品素材说明](docs/商品素材说明.md)。
...
UI 调整后可运行 `node scripts/verify-ui.mjs` 检查设计令牌、对比度和模板结构。`scripts/check-preview.cjs` 接受与截图脚本相同的 Playwright 包路径和浏览器路径，检查 3 种手机宽度和 4 种页面状态；这些仍是浏览器转换预览。
...
商品按包购买，每包增加 20 支库存。初始余额 100 元，每日补给 20 元；品牌烟参考价为中华硬 45、荷花 32、玉溪软 23、白沙硬 6 元/包，原创雨后/夜班分别 20/30 元，留白免费。￥ / 元仅表示虚拟余额，无真实支付或提现。旧存档余额、库存不变。

钱包提供每日三次广告奖励：50、100、200 元，北京时间重置。默认本地模式使用带标识的模拟观看弹窗。API 模式需要显式启用开发演示或配置广告位；生产禁止模拟。分享奖励按用户决定取消。详见 [广告奖励接入](docs/广告奖励接入.md)。
